...
def crawl(url, s):
    """ 抓取每一个搜索结果 """
    try:
        url = get_linkedin_url(url, copy.deepcopy(s)).replace('cn.linkedin.com', 'www.linkedin.com')    # Linkedin用户链接
...
if __name__ == '__main__':
    # 登录
    s = login(laccount='104758993@qq.com', lpassword='2311Kpan')
    ...
    while len(url) > 0 and failure < 10:
        ...
        if resp.status_code == 200:
            # 提取链接
            resp.encoding = 'utf-8'
            hrefs = list(set(re.findall('"(http://www\.baidu\.com/link\?url=.*?)"', resp.text)))  # 一页有10个搜索结果
            for href in hrefs:
                # 爬取每一个链接
            ...
            failure += 2
            print('search failed: %s' % resp.status_code)
